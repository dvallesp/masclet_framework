...
def stellar_mass_function(it, path='', L = 40., 
                          figsize = (5,4), ylim = (1e-6, 1), dpi = 300, 
                          logM_min = 7, logM_max = 12, Nbins = 20,
                          color = 'deepskyblue', linewidth = 3, alpha = 0.7, axlabelsize = 'large',
    ...
    ax.plot(10**Mbins[phi_intervals > 0], phi_intervals[phi_intervals > 0], color = color, alpha = alpha, label = f'z = {zeta:.1f}', linewidth = linewidth)
...
def cosmic_SFR_density(init_it, final_it, step, path='', L = 40.,
    ...
    ax.plot(z_array, np.log10(SFRdens), color = color, label = 'SFRD', linewidth = linewidth, alpha = alpha)
